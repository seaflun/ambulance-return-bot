...
1. NAS 可驗證的 Worker 心跳，作為唯一在線判定。
2. 與長工作分離的遠端更新命令收件器，優先確認命令已收到，但延後實際更新。
...
### 1. Worker health heartbeat

Worker 啟動後建立獨立背景心跳，每 10 秒執行一次。它先以原子寫入更新 `%LOCALAPPDATA%\AmbulanceReturnBot\worker_heartbeat.json`，再以既有 `X-Worker-Token` 對 NAS 送出健康資料。
...
NAS 以每台 Worker 最新一筆資料保存。收到時間不超過 45 秒顯示「在線」，超過 45 秒顯示「離線」。後台須分開顯示：NAS 後台版本、最後心跳版本/時間/狀態、最後任務回報版本/時間。最後任務回報永遠不能再冒充在線證據。

### 2. 高優先遠端更新命令收件器

新增專職協調器，每 10 秒以短逾時拉取 `/worker/remote-update`，與帳密同步、案件查詢和 Selenium 勤務分離。它的責任只有三件事：
...
這項分離的預期效果是：即使 Selenium 查詢持續數分鐘，更新命令仍可在約 10 秒內顯示「已收到／等待勤務完成」，而非被誤判為公務電腦離線。
...
- heartbeats 的 schema、原子寫入、Token 驗證、45 秒 online/offline 和管理頁版本分離；
- 遠端更新收件器即使主 Worker 正在長案件查詢時仍能回報命令已收到；
