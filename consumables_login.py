...
def login_acs_and_get_driver(
    ...
    account_text, password_text = _load_acs_credentials()
...
def _load_acs_credentials() -> tuple[str, str]:
    account = os.getenv("ACS_ACCOUNT", "").strip()
    password = os.getenv("ACS_PASSWORD", "")
    if not account or not password:
        raise RuntimeError("找不到一站通耗材帳密，請設定 ACS_ACCOUNT / ACS_PASSWORD。")
...
def _find_consumable_detail_href(driver: webdriver.Chrome, request: AmbulanceReturnRequest) -> str:
    ...
    for item in candidates:
        ...
        score = 0
        score += _consumable_sid_score(request.case_id, sid)
...
def _consumable_sid_score(case_id: str, sid: str) -> int:
    ...
    return 10 if any(fragment and fragment in sid_digits for fragment in fragments) else 0
...
def _case_id_sid_fragments(case_id: str) -> list[str]:
    ...
    if len(digits) < 14:
        return []
    fragments = [digits[8:14]]
    if len(digits) >= 17:
        fragments.append(digits[8:-3])
